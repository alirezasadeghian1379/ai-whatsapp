...
export interface SmsProvider{send(input:{to:string;body:string;sender?:string|null}):Promise<SmsResult>}
class MockSmsProvider implements SmsProvider{async send(){return{ok:true as const,messageId:`sms_test_${crypto.randomUUID()}`}}}
class UnconfiguredSmsProvider implements SmsProvider{constructor(private name:string){}async send(){return{ok:false as const,error:`اتصال عملی ${this.name} هنوز توسط مدیر سیستم پیکربندی نشده است.`}}}
