import makeWASocket, { Browsers, DisconnectReason, useMultiFileAuthState, type WASocket, type WAMessage } from "@whiskeysockets/baileys";
...
type Runtime = { socket?: WASocket; state: string; qr: string | null; starting?: Promise<void>; reconnectTimer?: ReturnType<typeof setTimeout> };
...
async function updateConnection(instanceName: string, state: string, phone?: string) {
  ...
  await db.whatsAppSession.update({ where: { id: session.id }, data: { status, phoneNumber: phone || session.phoneNumber, connectedAt: status === "CONNECTED" ? (session.connectedAt || new Date()) : session.connectedAt, lastSeenAt: new Date() } });
  if (status !== session.status && (status === "CONNECTED" || status === "DISCONNECTED")) {
    await db.notification.create({ data: { userId: session.userId, type: status === "CONNECTED" ? "whatsapp.connected" : "whatsapp.disconnected", title: status === "CONNECTED" ? "واتساپ متصل شد" : "اتصال واتساپ قطع شد", body: session.displayName || phone || "واتساپ", href: "/dashboard/whatsapp" } });
...
async function boot(instanceName: string, runtime: Runtime) {
  ...
  socket.ev.on("messages.upsert", async ({ messages, type }) => {
    ...
    for (const message of messages) {
      ...
      if (body && remoteJid) await recordWhatsAppMessage({ externalId: instanceName, messageId: message.key.id, remoteJid, originalJid: originalJid || undefined, fromMe: !!message.key.fromMe, pushName: message.pushName, body, timestamp: Number(message.messageTimestamp || 0) || undefined });
    }
  });
}
...
export async function startBaileysSession(instanceName: string, force = false) {
  ...
  if (!runtime) { runtime = { state: "connecting", qr: null }; runtimes.set(instanceName, runtime); }
  if (runtime.state === "close" && !runtime.starting) force = true;
  if (force) { runtime.socket?.end(undefined); runtime.socket = undefined; runtime.starting = undefined; }
  if (!runtime.socket && !runtime.starting) runtime.starting = boot(instanceName, runtime).finally(() => { runtime!.starting = undefined; });
...
export async function logoutBaileysSession(instanceName: string) {
  ...
  clearTimeout(runtime?.reconnectTimer);
  try { await runtime?.socket?.logout(); } catch { runtime?.socket?.end(undefined); }
