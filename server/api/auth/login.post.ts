...
const schema=z.object({identity:z.string().trim().min(3),password:z.string().min(8),remember:z.boolean().default(false)});
...
export default defineEventHandler(async event=>{
  ...
  if(!user||!await compare(parsed.data.password,user.passwordHash))throw createError({statusCode:401,statusMessage:"ایمیل، موبایل یا رمز عبور نادرست است."});
