...
export default defineEventHandler(async (event) => {
  ...
  const sessions = await databaseAction(() => db.whatsAppSession.findMany({ where: { userId: String(auth.sub) }, orderBy: { createdAt: "desc" } }));
  const synced = await Promise.all(sessions.map(async (session) => {
    ...
    return databaseAction(() => db.whatsAppSession.update({ where: { id: session.id }, data: { status, connectedAt: status === "CONNECTED" ? (session.connectedAt ?? new Date()) : session.connectedAt, lastSeenAt: new Date() } }));
  }));
