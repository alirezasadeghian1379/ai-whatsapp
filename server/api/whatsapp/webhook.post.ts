...
export default defineEventHandler(async (event) => {
  ...
  if (!config.whatsappWebhookSecret || suppliedSecret !== config.whatsappWebhookSecret) throw createError({ statusCode: 401, statusMessage: "Webhook unauthorized" });
  ...
  if (eventName === "MESSAGES_UPSERT") {
    ...
    if (remoteJid && !remoteJid.endsWith("@g.us") && remoteJid !== "status@broadcast") {
      ...
      if (phone && body) {
        const contact = await db.contact.upsert({ where: { userId_phone: { userId: session.userId, phone } }, update: { name: item?.pushName || undefined, whatsappId: remoteJid, lastInteractionAt: new Date() }, create: { userId: session.userId, phone, name: item?.pushName || null, whatsappId: remoteJid, lastInteractionAt: new Date() } });
        const conversation = await db.conversation.upsert({ where: { sessionId_contactId: { sessionId: session.id, contactId: contact.id } }, update: { lastMessageAt: new Date(), unreadCount: key?.fromMe ? undefined : { increment: 1 } }, create: { userId: session.userId, sessionId: session.id, contactId: contact.id, lastMessageAt: new Date(), unreadCount: key?.fromMe ? 0 : 1 } });
        const data = { conversationId: conversation.id, externalId: key?.id || null, direction: key?.fromMe ? "OUTBOUND" : "INBOUND", body, status: key?.fromMe ? "SENT" : "RECEIVED", sentAt: new Date(Number(item?.messageTimestamp || 0) * 1000 || Date.now()) };
        const saved = key?.id ? await db.message.upsert({ where: { externalId: key.id }, update: { status: data.status }, create: data }) : await db.message.create({ data });
        if (!key?.fromMe) {
          void dispatchUserWebhooks(session.userId, "message.received", { messageId: saved.id, conversationId: conversation.id, contact: { id: contact.id, phone, name: contact.name }, body });
          void runAutoReply({ userId: session.userId, sessionExternalId: session.externalId, conversationId: conversation.id, phone, message: body });
        }
      }
    }
  }
