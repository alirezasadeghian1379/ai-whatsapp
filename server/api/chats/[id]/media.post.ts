...
export default defineEventHandler(async event => {
    ...
    if (!file?.data || !file.filename || !file.type || !allowed.has(file.type)) throw createError({
        statusCode: 422,
        statusMessage: "فقط تصویر، PDF یا فایل متنی مجاز است."
    })
    if (file.data.length > 5 * 1024 * 1024) throw createError({
        statusCode: 413,
        statusMessage: "حجم فایل نباید بیشتر از ۵ مگابایت باشد."
    })
    ...
    if (conversation.session.status !== "CONNECTED") throw createError({
        statusCode: 409,
        statusMessage: "واتساپ این گفتگو متصل نیست."
    })
    const sent = await getWhatsAppProvider().sendMedia(conversation.session.externalId, conversation.contact.phone, {
        data: file.data,
        mimeType: file.type,
    ...
    await writeFile(join(mediaDir, storedName), file.data)
