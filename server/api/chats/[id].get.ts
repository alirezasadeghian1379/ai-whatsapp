...
export default defineEventHandler(async event=>{
 ...
 return{conversation:{...conversation,unreadCount:0,messages:conversation.messages.map(message=>unread.some(item=>item.id===message.id)?{...message,status:"READ"}:message)}}
})
