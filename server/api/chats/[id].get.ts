...
export default defineEventHandler(async (event) => {
  ...
  const conversation = await db.conversation.findFirst({ where: { id, userId: String(auth.sub) }, include: { contact: true, session: true, messages: { orderBy: { createdAt: "asc" }, take: 200 } } });
