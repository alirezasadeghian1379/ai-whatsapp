...
async function submit() {
  ...
  error.value = "";
  try {
    await $fetch("/api/auth/reset-password", {method: "POST", body: {token: String(route.query.token || ""), ...form}});
    done.value = true
  } catch (e: any) {
    error.value = e.data?.statusMessage || tr("تغییر رمز ناموفق بود.", "Could not reset password.")
  } finally {
    busy.value = false
  }
}</script>
<template>
  <form @submit.prevent="submit"><h1 class="text-3xl font-black">
    {{ tr('انتخاب رمز جدید', 'Choose a new password') }}</h1>
    <p class="muted mb-7 mt-3">
      {{ tr('رمز جدید باید حداقل ۸ نویسه باشد.', 'Your new password must be at least 8 characters.') }}</p>
    <div v-if="error" class="mb-4 rounded-xl bg-red-50 p-4 text-sm font-bold text-red-700">{{ error }}</div>
    <div v-if="done" class="rounded-xl bg-emerald-50 p-4 text-sm font-bold text-emerald-700">
