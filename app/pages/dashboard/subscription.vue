...
const busy = ref(""), error = ref(""), provider = ref("zarinpal");
...
const unavailable = (plan: any) => data.value?.subscription?.planId === plan.id || (!Number(plan.price) && (data.value?.freeUsed || Number(data.value?.subscription?.plan?.price) > 0));
...
async function buy(plan: any) {
  ...
  try {
    const r = await $fetch<any>("/api/payments/create", {
      method: "POST",
      body: {planId: plan.id, provider: provider.value}
    });
    if (r.activated) {
      await refresh();
...
<template>
  <div>
    ...
    <template v-else>
      <section v-if="data?.subscription" class="grid gap-5 lg:grid-cols-[1.2fr_.8fr]">
        ...
        <article class="surface p-6"><h2 class="font-black">{{ tr('محدودیت‌های پلن', 'Plan limits') }}</h2>
          <ul class="mt-5 space-y-4">
            <li v-for="x in [`${formatNumber(data.subscription.plan.maxWhatsAppConnections)} WhatsApp`,`${formatNumber(data.subscription.plan.maxMessages)} ${tr('پیام','messages')}`,`${formatNumber(data.subscription.plan.maxAIRequests)} AI`,`${formatNumber(data.subscription.plan.maxWebhooks)} Webhook` ]"
      ...
      <section class="surface mt-5 flex flex-wrap items-center gap-3 p-4"><b
          class="text-sm">{{ tr('درگاه پرداخت', 'Payment gateway') }}</b><select v-model="provider"
      ...
      <section class="mt-5 grid gap-4 md:grid-cols-3">
        <article v-for="plan in data?.plans" :key="plan.id" class="surface p-6"
          ...
          <button class="btn w-full" :class="unavailable(plan)?'btn-secondary':'btn-primary'"
                  :disabled="busy===plan.id||unavailable(plan)" @click="buy(plan)">
      ...
      <section class="surface mt-5 overflow-hidden">
        ...
        <div v-for="order in data?.orders" :key="order.id" class="grid gap-3 border-b p-5 sm:grid-cols-4">
          <code>{{ order.id.slice(-10) }}</code><span>{{ formatDate(order.createdAt, {dateStyle: 'medium'}) }}</span><b>{{ formatNumber(order.amount) }}
          {{ tr('تومان', 'IRR') }}</b><span class="badge w-fit"
                                            :class="order.status==='PAID'?'bg-emerald-50 text-emerald-600':'bg-amber-50 text-amber-600'">{{ order.status }}</span>
        </div>
      </section>
    </template>
  </div>
