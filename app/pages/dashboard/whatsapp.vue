...
function date(value: string | null) { return formatDate(value, { dateStyle: "medium", timeStyle: "short" }); }
...
<template>
  <div>
    ...
    <div v-if="actionError || loadError" class="mb-5 flex items-start gap-3 rounded-2xl border border-red-200 bg-red-50 p-4 text-sm font-bold text-red-700 dark:border-red-900 dark:bg-red-950/40 dark:text-red-300"><AlertCircle class="mt-0.5 shrink-0" :size="19" /><span>{{ actionError || apiError(loadError) }}</span></div>
    ...
    <div v-else class="grid gap-5 lg:grid-cols-2">
      <section v-for="session in sessions" :key="session.id" class="surface p-6 transition hover:-translate-y-0.5 hover:shadow-xl">
        ...
        <dl class="mt-6 grid gap-4 rounded-2xl bg-slate-50 p-5 text-sm dark:bg-slate-800 sm:grid-cols-2"><div><dt class="text-slate-400">Provider</dt><dd class="mt-2 font-bold">{{ session.provider === 'baileys' ? 'Baileys (Node.js)' : session.provider }}</dd></div><div><dt class="text-slate-400">{{ tr('آخرین بروزرسانی', 'Last update') }}</dt><dd class="mt-2 font-bold">{{ date(session.lastSeenAt) }}</dd></div></dl>
