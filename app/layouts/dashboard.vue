...
<template>
  <div class="min-h-screen bg-slate-50 dark:bg-slate-950">
    <div v-if="mobileOpen" class="fixed inset-0 z-30 bg-slate-950/50 backdrop-blur-sm lg:hidden"
         @click="mobileOpen=false"/>
    <aside
        class="fixed inset-y-0 start-0 z-40 flex w-72 flex-col border-e bg-white p-4 transition-transform duration-300 dark:bg-slate-900"
        :class="mobileOpen?'translate-x-0':'ltr:-translate-x-full rtl:translate-x-full lg:translate-x-0!'">
      <div class="flex h-16 items-center justify-between px-2">
        <AppLogo/>
        <button class="icon-btn lg:hidden" @click="mobileOpen=false">
          <X :size="18"/>
        </button>
      </div>
      <p class="px-3 pb-2 pt-5 text-[11px] font-bold uppercase tracking-widest text-slate-400">
        {{ tr('فضای کاری', 'Workspace') }}</p>
      <nav class="space-y-1 overflow-y-auto">
        <NuxtLink v-for="item in items" :key="item.to" :to="item.to"
                  ...
                  class="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-bold text-slate-500 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-800">
      ...
      <div class="mt-auto space-y-2">
        <NuxtLink v-if="isAdmin" to="/admin"
                  class="flex items-center gap-3 rounded-xl bg-violet-50 px-3 py-2.5 text-sm font-bold text-violet-700 dark:bg-violet-500/10 dark:text-violet-300">
          <ShieldCheck :size="18"/>
          {{ tr('پنل مدیریت', 'Admin panel') }}
        </NuxtLink>
        <NuxtLink to="/dashboard/help"
                  class="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800">
          <HelpCircle :size="18"/>
          {{ t('help') }}
        </NuxtLink>
    ...
    <div class="lg:ps-72">
      <header
          class="sticky top-0 z-20 flex h-20 items-center gap-3 border-b bg-white/80 px-4 backdrop-blur-xl dark:bg-slate-950/80 sm:px-6 lg:px-8">
