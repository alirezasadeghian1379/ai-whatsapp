  ...
  <section class="surface p-6 transition hover:-translate-y-0.5 hover:shadow-xl">
    ...
    <dl class="mt-6 grid gap-4 rounded-2xl bg-slate-50 p-5 text-sm dark:bg-slate-800 sm:grid-cols-2">
      <div>
        ...
        <dd class="mt-2 font-bold">{{ session.provider === 'baileys' ? 'Baileys (Node.js)' : session.provider }}</dd>
      </div>
      <div>
        <dt class="text-slate-400">{{ tr('آخرین بروزرسانی', 'Last update') }}</dt>
        <dd class="mt-2 font-bold">{{ formattedDate }}</dd>
      </div>
    </dl>
    <div class="mt-6 flex flex-wrap gap-2">
      <button v-if="session.status==='CONNECTED'" type="button" class="btn btn-primary" :disabled="busy"
              @click="$emit('send',session.id)">
        <Send :size="17"/>
        {{ tr('ارسال پیام', 'Send message') }}
      </button>
      <button v-else type="button" class="btn btn-primary" :disabled="busy" @click="$emit('qr',session.id)">
        <QrCode :size="17"/>
        {{ tr('نمایش QR', 'Show QR') }}
      </button>
      <button v-if="session.status==='CONNECTED'" type="button" class="btn btn-secondary" :disabled="busy"
      ...
      <button type="button"
        ...
        <Trash2 :size="17"/>
        {{ tr('حذف اتصال', 'Delete connection') }}
      </button>
    </div>
  </section>
</template>
